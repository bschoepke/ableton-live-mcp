...
ABLETON_MCP_INSTRUCTIONS = (
    ...
    "M4L: free native/web/mixed UI hot-reloads; wait_status/compact_result+cmd. Reuse hosts/IDs; host_runtime_version; avoid renderer/fps; cleanup dry-run. Supports preflight, sidecar+slim set/status, web_reload UI-only, slim web-state, UDP hints, throttled fallback wakes, load:false/set/status skip build, direct status polls, host_not_woken=no ack, midiin+midiparse, origin rect/openrect, bounds, ui_bindings, agent-settable UI, telemetry report:false, ack protection/state throttles, web assets/source_path, status_state_keys/_only diag, set_silent/batch/list vals, audio buses, jweb/jbrowser aliases; audio-reactive web: clock/prove signal telemetry+visual delta. UI hang/high CPU: unload web devices/restart, validate. No web ack/shrink: simplify/new host. "
    "Avoid broad dumps. "
