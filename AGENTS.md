...
After a generated spec loads, the host should also create and explicitly start a hidden dynamic low-priority `qmetro` connected back into the JS object. Dynamic objects have proven more reliable than static startup triggers in stressed Live sets, and this keeps later `set`/`status` commands and web-read retries moving after the initial load succeeds.

Generated hosts also expose a hidden Live parameter named `Agent Poll`. After writing a command file, the Remote Script should toggle that parameter on the target generated device when a target track is available; the parameter output is a reliable M4L-side command-file trigger and should be preferred over UDP/timer wakeups in stressed sets.
