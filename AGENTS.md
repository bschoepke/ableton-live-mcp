...
If the capture result reports `warning: blank_capture` or postprocess content stats show `blank: true`, treat that screenshot as invalid validation evidence. A locked laptop, sleeping display, or locked OS session can make otherwise valid Ableton-window capture return a blank image; stop retrying capture backends in that case, ask the user to unlock/wake the display, and record the blocker. Otherwise retry with the other backend, bring the Ableton Live window onto a capturable display/space, or continue with status-only evidence while recording the visual validation blocker. Do not claim UI visibility or audio-reactive visuals from a blank capture.
