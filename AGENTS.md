...
When a host AMXD is loaded after its command file has already been written, the host should use `live.thisdevice` plus a self-starting low-priority `metro` to trigger command-file reads after Max finishes loading the M4L device. Avoid applying large dynamic specs directly inside JS `loadbang`; that can race the static polling chain in heavy sets. The same polling bang should also service pending web reads so browser retries do not depend solely on Max JS `Task` scheduling.
