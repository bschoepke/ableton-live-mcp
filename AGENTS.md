...
Preflight presentation geometry is advisory. Use it to catch obvious clipping risks such as presentation bounds exceeding the declared device width/height or very tall device viewports that need visual capture, but do not reject unusual free-form UIs solely because they are large, scrollable, horizontally wide, or use custom native/web layouts.
