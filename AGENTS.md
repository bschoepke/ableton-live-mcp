...
When a host AMXD is loaded after its command file has already been written, a delayed static bang should trigger the first command-file read after Max finishes loading the patcher. Avoid applying large dynamic specs directly inside JS `loadbang`; that can race the static polling chain in heavy sets.
