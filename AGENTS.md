...
After a generated spec loads, the host should also create a hidden dynamic low-priority polling `metro` connected back into the JS object. Dynamic objects have proven more reliable than static startup triggers in stressed Live sets, and this keeps later `set`/`status` commands and web-read retries moving after the initial load succeeds.
